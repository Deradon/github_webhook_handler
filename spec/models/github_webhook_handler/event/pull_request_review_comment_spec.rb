...
module GithubWebhookHandler
  RSpec.describe Event::PullRequestReviewComment, type: :model do
    subject { described_class.new(payload: payload) }

    let(:payload) { JSON.parse(File.read(json_path)) }
    let(:json_path) { "spec/fixtures/github_events/pull_request_review_comment.json" }
    ...
    describe ".webhook_event_name" do
      ...
      it { is_expected.to eq("pull_request_review_comment") }
    end
  end
end
