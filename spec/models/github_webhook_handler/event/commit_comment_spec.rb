...
module GithubWebhookHandler
  RSpec.describe Event::CommitComment, type: :model do
    subject { described_class.new(payload: payload) }

    let(:payload) { JSON.parse(File.read(json_path)) }
    let(:json_path) { "spec/fixtures/github_events/commit_comment.json" }
    ...
    context "without comment" do
      before { payload.merge!("comment" => nil ) }
      it { is_expected.not_to be_valid }
    end
    ...
    describe ".webhook_event_name" do
      ...
      it { is_expected.to eq("commit_comment") }
    end
  end
end
